...
def main(*filenames):
    ...
    for ii in range(0, 3):
        if len(results[ii]) != 0:
            meet_info = split_no_brackets(results[ii][0], '.')
    ...
    if len(meet_info) > 3:  # Then this is probably meet information
        ...
        # If more than one results file has been passed, then see if we can
            ...
            for filename in filenames[1:]:
                ...
                for ii in range(0, 5):
                    ...
                    if len(meet_info) > 3:  # Looks like we've found the meet info
                        break
                if meet_info != []:
                    # Men and womens results are named under different meets
