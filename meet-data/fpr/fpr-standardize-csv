...
def parse_sheet(sheet):
    ...
    for ii in iterable:
        ...
        if 'Результаты' in text:
            ...
            continue
        elif 'Место' in text or 'Фамилия. Имя' in text:  # Another header
            continue
        elif 'Абсолютное' in text:  # Skip until we find another header
            while iterable is not None and not is_header(line):
                try:
                    ii = next(iterable)
                    line = sheet[ii]
