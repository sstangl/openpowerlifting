...
pub enum PossiblyOwnedFilter<'db> {
    Borrowed(&'db Filter),
    Owned(Filter),
}
...
impl Selection {
    /// Uses the new StaticCache system to return a sorted, uniqued list.
    ///
    /// TODO: This uses an Option because I'm still hooking up the cache,
    /// but eventually this should cover all use-cases. Then we can delete
    /// to_filter() and make this almost-infallible (except OOM).
    pub fn from_static_cache<'db>(
        &self,
            ...
            && self.year == YearSelection::AllYears
        {
            ...
            let sorted_uniqued = match self.equipment {
            ...
            return Some(sorted_uniqued);
        }
        None
    }

    /// Uses the old Filter system to return an unsorted, un-uniqued filter.
    pub fn to_filter<'db>(&self, opldb: &'db opldb::OplDb) -> PossiblyOwnedFilter<'db> {
        // An equipment filter is always used.
        let filter_equipment: &Filter =
            opldb.get_filter(self.equipment.to_cached_filter());
        let mut cur = PossiblyOwnedFilter::Borrowed(filter_equipment);

        // If the selection is default, only the equipment filter is used.
        if *self == Selection::new_default() {
            return cur;
        }

        // Apply the sex filter.
        if let Some(c) = self.sex.to_cached_filter() {
            cur = PossiblyOwnedFilter::Owned(cur.intersect(opldb.get_filter(c)));
        }

        // Apply the year filter.
        if let Some(c) = self.year.to_cached_filter() {
            cur = PossiblyOwnedFilter::Owned(cur.intersect(opldb.get_filter(c)));
        }
        ...
        if self.weightclasses != WeightClassSelection::AllClasses {
            let (lower, upper) = self.weightclasses.to_bounds();
